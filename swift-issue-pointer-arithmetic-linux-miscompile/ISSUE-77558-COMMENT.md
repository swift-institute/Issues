...
**Status**: revised three times on 2026-05-11. First revision: lead with empirically-verified Linux release firing path; caveat prior macOS-standalone observation. Second revision (orchestrator review): three wording nits + post-able body extracted to sibling `ISSUE-77558-COMMENT-BODY.md`. **Third revision (pre-publication audit by independent fresh chat)**: fixed wrong Docker tag `swiftlang/swift:6.3.1-RELEASE` → `swift:6.3-jammy` throughout (the prior tag does not exist on Docker Hub); replaced model-inherited SIL excerpt with structural-only description (real `swiftc -O -emit-sil` confirms the structural pattern but value numbers vary by configuration); restructured trigger-characterization bullets into "empirically isolated" (4 items, INVESTIGATION-ARC Round 3) vs "model-converged hypotheses" (6 items, not individually re-verified); other clarifications. Awaiting explicit user YES before `gh issue comment 77558 -R swiftlang/swift --body-file <sibling-body-file>`.
