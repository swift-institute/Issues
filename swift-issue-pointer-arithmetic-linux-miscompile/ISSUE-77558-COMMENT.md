...
**Status**: revised four times on 2026-05-11. First revision: lead with empirically-verified Linux release firing path; caveat prior macOS-standalone observation. Second revision (orchestrator review): three wording nits + post-able body extracted to sibling `ISSUE-77558-COMMENT-BODY.md`. Third revision (first pre-publication audit by independent fresh chat): fixed wrong Docker tag `swiftlang/swift:6.3.1-RELEASE` → `swift:6.3-jammy` throughout (the prior tag does not exist on Docker Hub); replaced model-inherited SIL excerpt with structural-only description; restructured trigger-characterization bullets into "empirically isolated" vs "model-converged hypotheses". **Fourth revision (second pre-publication audit)**: added sentence after the candidate-fix-commits list noting they target array-literal lowering vs Schreib's tuple-storage shape; softened workaround framing to acknowledge the `for v in values { _ = v }` loop variant is in the same class as Schreib's reported `print(start)` (not a literally-equivalent incantation). The second audit returned POST-AS-IS with these as optional tightenings; both applied. Awaiting explicit user YES before `gh issue comment 77558 -R swiftlang/swift --body-file <sibling-body-file>`.
