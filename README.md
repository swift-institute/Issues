...
- [`swift-issue-borrowing-actor-closure/`](swift-issue-borrowing-actor-closure/) — `MoveOnlyTypeEliminator` crashes when an actor method captures a `borrowing` class parameter in a non-escaping closure ([`swift-institute/Issues#3`](https://github.com/swift-institute/Issues/issues/3)); verified on Swift 6.2.3.
- [`swift-issue-emit-module-noncopyable-sequence/`](swift-issue-emit-module-noncopyable-sequence/) — module emission rejects a valid `~Copyable` `Sequence` conformance ([Issues#4](https://github.com/swift-institute/Issues/issues/4)); source evidence retained.
- [`swift-issue-embedded-typed-throws-crash/`](swift-issue-embedded-typed-throws-crash/) — Embedded Swift typed-throws protocol conformances crash compilation ([Issues#5](https://github.com/swift-institute/Issues/issues/5)); source evidence retained.
- [`swift-issue-copypropagation-nonescapable-mark-dependence/`](swift-issue-copypropagation-nonescapable-mark-dependence/) — CopyPropagation `~Escapable` coroutine-yield crash ([Issues#6](https://github.com/swift-institute/Issues/issues/6)); **fixed** in Swift 6.3 (Xcode 26.4).
- [`swift-issue-inlinearray-deinit-value-generic/`](swift-issue-inlinearray-deinit-value-generic/) — a value-generic `InlineArray` skips cross-module `~Copyable` deinitialization ([Issues#7](https://github.com/swift-institute/Issues/issues/7)); source evidence retained.
- [`swift-issue-irgen-async-typed-throws-noncopyable/`](swift-issue-irgen-async-typed-throws-noncopyable/) — async typed throws with a nested generic error crashes IRGen ([Issues#8](https://github.com/swift-institute/Issues/issues/8)); source evidence retained.
- [`swift-issue-irgen-typed-throws-nested-error-generic/`](swift-issue-irgen-typed-throws-nested-error-generic/) — a typed-throws closure using a nested generic error crashes IRGen ([Issues#9](https://github.com/swift-institute/Issues/issues/9)); source evidence retained.
- [`swift-issue-nested-generic-subscript-performance/`](swift-issue-nested-generic-subscript-performance/) — nested value-generic subscript access has debug-build overhead ([Issues#10](https://github.com/swift-institute/Issues/issues/10)); benchmark and SIL evidence retained.
- [`swift-issue-nested-copyable-extension-bug/`](swift-issue-nested-copyable-extension-bug/) — same-file extension of a deeply nested `~Copyable` type corrupts name lookup ([Issues#11](https://github.com/swift-institute/Issues/issues/11)); forensic evidence retained.
- [`swift-issue-noncopyable-sequence-conformance/`](swift-issue-noncopyable-sequence-conformance/) — an associated-type conformance leaks `Copyable` into a nested `~Copyable` type ([Issues#12](https://github.com/swift-institute/Issues/issues/12)); source evidence retained.
...
- [`swift-issue-extension-table-uint16-datalength-overflow/`](swift-issue-extension-table-uint16-datalength-overflow/) — the serialized extension-lookup-table `dataLength` is written and read as `uint16_t`, so it overflows once one type's extensions exceed 65535 bytes of table data (`EmitKeyDataLength`, `Serialization.cpp:239`): **crash** (signal 6) on +assertions toolchains, **silent module truncation** on release (valid member references then fail as `has no member 'Artikel N'`). Present 6.3.3 (silent) → 6.5-dev (asserts), unfixed. Natural trigger: a statute-book namespace with ~700 per-article `extension`s (the real package overflows at 736). **FILED — [swiftlang/swift#90319](https://github.com/swiftlang/swift/issues/90319)** (2026-07-01). Upstream fix: widen the on-disk width to `uint32`/VBR, or at minimum diagnose instead of silently truncating on release.
...
- [`swift-issue-spm-identity-conflict-path-enumeration-hang/`](swift-issue-spm-identity-conflict-path-enumeration-hang/) — SwiftPM graph-load hang: one package identity reachable under two canonical locations (e.g. `.git` vs bare spelling across an exact-string mirror table) enters `createResolvedPackages`' conflicting-identity branch, whose `findAllTransitiveDependencies` (PR #8390) enumerates ALL paths with no visited set — exponential on dense DAGs, effectively non-terminating at institute scale. Present Swift 6.2 → 6.3.2 → `main`, unfixed. Workaround: dual-spelling mirror coverage + uniform `.git` manifest spellings; verify with `swift build`, never `show-dependencies` (its tree dumper is independently exponential). Terminal record — not filed. *(topology reproducer via `evidence/gen-synthetic.sh`; no `withKnownIssue` harness — same deferral as the spm-planning-build-stall entry)*
...
- [`swift-issue-property-view-tag-constraint-cross-module/`](swift-issue-property-view-tag-constraint-cross-module/) — cross-module constrained `Property.View` extensions are incorrectly diagnosed as redeclarations ([Issues#13](https://github.com/swift-institute/Issues/issues/13)); exact multi-module evidence retained.
- [`swift-issue-rawlayout-deinit-cross-package/`](swift-issue-rawlayout-deinit-cross-package/) — `@_rawLayout` element deinitialization is skipped across package boundaries ([Issues#14](https://github.com/swift-institute/Issues/issues/14)); exact package evidence retained.
- [`swift-issue-sil-verifier-read-escapable-lifetime/`](swift-issue-sil-verifier-read-escapable-lifetime/) — `_read` yielding a `~Escapable` lifetime-dependent value fails SIL ownership verification ([Issues#15](https://github.com/swift-institute/Issues/issues/15)).
- [`swift-issue-silgen-pack-expansion-cross-module/`](swift-issue-silgen-pack-expansion-cross-module/) — a cross-module parameter-pack expansion call crashes SILGen ([Issues#16](https://github.com/swift-institute/Issues/issues/16)).
- [`swift-issue-silgen-property-wrapper-noncopyable/`](swift-issue-silgen-property-wrapper-noncopyable/) — cross-module property-wrapper access on a `~Copyable` type crashes SILGen ([Issues#17](https://github.com/swift-institute/Issues/issues/17)).
- [`swift-issue-testing-suite-discovery-generic-specialization/`](swift-issue-testing-suite-discovery-generic-specialization/) — Swift Testing omits suites declared in generic-specialization extensions ([Issues#18](https://github.com/swift-institute/Issues/issues/18)).
- [`swift-issue-testing-xcode-nested-suite-filter/`](swift-issue-testing-xcode-nested-suite-filter/) — Xcode gutter actions report zero tests for deeply nested Swift Testing suites ([Issues#19](https://github.com/swift-institute/Issues/issues/19)).
- [`swift-issue-typed-throws-autoclosure-inference/`](swift-issue-typed-throws-autoclosure-inference/) — `@autoclosure` prevents `Never` inference for typed throws ([Issues#20](https://github.com/swift-institute/Issues/issues/20)).
- [`swift-issue-windows-existential-crash/`](swift-issue-windows-existential-crash/) — Windows debug-info mangling crashes for a cross-package existential type ([Issues#21](https://github.com/swift-institute/Issues/issues/21)); both source identities are retained in one dossier.
- [`swift-issue-spm-repository-url-identity/`](swift-issue-spm-repository-url-identity/) — parent record for SwiftPM repository-URL identity and redirect behavior ([Issues#22](https://github.com/swift-institute/Issues/issues/22)); the three remote fixtures remain active test infrastructure.
