...
// `read` accessor with `shared` linkage — which is therefore NOT carried in
// that module's `.swiftmodule`. When a CONSUMER module declares a SECOND
// conformance of the same type to a protocol with an equivalent requirement,
// the consumer's witness references that accessor and emits it as a bodyless
// `shared [serialized]` SIL function. Wherever SIL verification runs
// (+Asserts toolchains, Embedded, Windows, `-sil-verify-all`), swift-frontend
// aborts:
//
...
//
// On a NoAsserts RELEASE toolchain the malformed SIL is emitted silently — the
// bug is latent there, not absent, which is why stock macOS/Linux release legs
// pass while +Asserts nightly, Embedded and Windows legs fail on identical
// code.
//
// DIFFERENTIATION
// ---------------
// This is the same "bodyless `shared [serialized]` coroutine crossing a module
// boundary" verifier class as the sibling entries
// `swift-issue-noncopyable-assoctype-never-bodyless-witness` and
// `swift-issue-sillinker-borrowed-protocol-default-shared-coroutine-abort`,
// but a DIFFERENT trigger: there the bodyless function is a PROTOCOL EXTENSION
// DEFAULT (`Body == Never`, or a `@_borrowed` requirement's default); here
// there is no extension default at all — the bodyless function is the
// CONCRETE type's own accessor, and what pulls it across the boundary is a
// second protocol conformance declared in the consumer.
//
...
// (checked 2026-07-30, 6.4-dev +assertions), so the module boundary is
...
// 6.4.x-snapshot-2026-07-23 and main-snapshot-2026-07-11 all abort), so
// `when:` is `{ true }`.

@Suite
struct NoncopyableAssoctypeSecondProtocolBodylessWitnessReproducer {
    ...
    static func bugFires() -> Bool? {
        ...
        let work = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("second-protocol-bodyless-witness-\(pid)")
    ...
    func reproducer() {
        ...
        withKnownIssue(
            """
            Bodyless `shared [serialized]` `read` accessor of a concrete type's \
            `~Copyable`-associated-type property, emitted into a consumer module \
            by a second protocol conformance and rejected wherever SIL \
            verification runs
            """,
            { #expect(fired == false) },
