...
// Standalone exit-code probe for the bodyless `shared [serialized]` `read`
// accessor of a CONCRETE conforming type, emitted into a consumer module that
// adds a SECOND conformance witnessed by that same property.
//
...
let work = URL(fileURLWithPath: NSTemporaryDirectory())
    .appendingPathComponent("second-protocol-bodyless-witness-probe-\(ProcessInfo.processInfo.processIdentifier)")
...
// Embedded / Windows configurations where verification is on by default; on a
// NoAsserts RELEASE toolchain the malformed SIL is emitted but never verified,
// so without the flag the bug is latent, not absent.
...
if importing.stderr.contains("Must have a construct to emit for")
    ...
    FileHandle.standardError.write(Data(
        "BUG FIRES: the consumer module aborted on the bodyless `Box.body.read` witness.\n".utf8
    ))
    exit(1)
}
